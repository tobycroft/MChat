 ...
 */
class Msg extends CommonController {
    ...
    public function clear_private_unread() {
        $fid = input('post.fid');
        $ret = MessageAction::clear_private_unread($this->uid, $fid);
        if ($ret['code'] == 0) {
            $this->succ($ret['data']);
        } else {
            $this->succ($ret['data']);
    ...
    public function clear_group_unread() {
        $gid = input('post.gid');
        $ret = MessageAction::clear_group_unread($this->uid, $gid);
        if ($ret['code'] == 0) {
            $this->succ($ret['data']);
        } else {
            $this->succ($ret['data']);
