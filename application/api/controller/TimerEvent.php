...
class TimerEvent extends CommonController
{
    // 最大运行时间
    ...
    {
        $group = GroupInfoModel::expiredGroup();
        try {
            Db::startTrans();
            foreach ($group as $v)
            {
                // 删除群成员
                GroupMemberModel::api_delByGid($v['id']);
                // 删除群
                GroupInfoModel::api_delete_byId($v['id']);
                // todo 删除群消息
            }
            TimerLogModel::api_insert('定时任务-解散群 执行成功');
            Db::commit();
            $this->succ('操作成功');
        } catch(Exception $e) {
            Db::rollback();
            TimerLogModel::api_insert('定时任务-解散群 执行失败，失败信息：' . sprintf('File: %s; Line: %s; Message: %s;' , $e->getFile() , $e->getLine() , $e->getMessage()));
            throw $e;
        }
    }
